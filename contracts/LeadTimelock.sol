...
contract LeadTimelock {
    ...
    mapping(address => uint) private percentages;       //10 = 1% 
    ...
    function getPercentage(address _beneficiary) external view returns(uint integer){
        return percentages[_beneficiary] / 10;
    }
    ...
    function _calculatePayment(address _beneficiary) private view returns(uint){
        ...
        return (percentages[_beneficiary] * balance) / ((scheduleList.length - nextSubtract) * (1000));
    }
